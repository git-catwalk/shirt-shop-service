...
class DefaultsServiceTest {
  ...
  @MockBean
  private DefaultsRepo repo;

  Defaults item1;
  Defaults item2;
  DefaultsService service;

  @BeforeEach
  void before(){
    EasyRandom generator = new EasyRandom();
    item1 = generator.nextObject(Defaults.class);
    item2 = generator.nextObject(Defaults.class);
    service = new DefaultsService(this.repo);
  }

  @Test
  void shouldSave(){
    Mockito.when(repo.save(Mockito.any(Defaults.class))).thenReturn(item1);
    Defaults data = service.save(Defaults.builder().build());
  ...
  void findAll(){
    ...
    List<Defaults> all = StreamSupport.stream(service.findAll().spliterator(), false).collect(Collectors.toList());
  ...
  void search(){
    ...
    Page<Defaults> page = new PageImpl<>(Arrays.asList(item1,item2));
    Mockito.when(repo.findAll(pageRequest)).thenReturn(page);
    List<Defaults> all = new ArrayList<>(service.search(null, pageRequest).getContent());
    Assertions.assertEquals(2, all.size());
  }
