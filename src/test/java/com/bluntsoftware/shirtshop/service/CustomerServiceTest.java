...
class CustomerServiceTest {
  ...
  void before(){
    ...
    item1 = generator.nextObject(Customer.class);
    item2 = generator.nextObject(Customer.class);
    service = new CustomerService(this.repo);
  }

  @Test
  void shouldSave(){
    Mockito.when(repo.save(Mockito.any(Customer.class))).thenReturn(item1);
    Customer data = service.save(Customer.builder().build());
  ...
  void findAll(){
    ...
    List<Customer> all = StreamSupport.stream(service.findAll().spliterator(), false).collect(Collectors.toList());
  ...
  void search(){
    ...
    Page<Customer> page = new PageImpl<>(Arrays.asList(item1,item2));
    Mockito.when(repo.findAll(pageRequest)).thenReturn(page);
    List<Customer> all = new ArrayList<>(service.search(null, pageRequest).getContent());
    Assertions.assertEquals(2, all.size());
  }
