...
class GarmentServiceTest {
  ...
  void before(){
    ...
    item1 = generator.nextObject(Garment.class);
    item2 = generator.nextObject(Garment.class);
    service = new GarmentService(this.repo);
  }

  @Test
  void shouldSave(){
    Mockito.when(repo.save(Mockito.any(Garment.class))).thenReturn(item1);
    Garment data = service.save(Garment.builder().build());
  ...
  void findAll(){
    ...
    List<Garment> all = StreamSupport.stream(service.findAll().spliterator(), false).collect(Collectors.toList());
  ...
  void search(){
    ...
    Page<Garment> page = new PageImpl<>(Arrays.asList(item1,item2));
    Mockito.when(repo.findAll(pageRequest)).thenReturn(page);
    List<Garment> all = new ArrayList<>(service.search(null, pageRequest).getContent());
    Assertions.assertEquals(2, all.size());
  }
